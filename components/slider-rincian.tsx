          ...
          >
            {media.map((_, i) => (
              <button
                ...
                aria-current={i === kini}
                onClick={() => setIndeks(i)}
                className="inline-flex min-h-[16px] min-w-[16px] items-center justify-center p-0.5 bg-transparent border-0 cursor-pointer sm:min-h-[28px] sm:min-w-[28px] sm:p-1"
              >
                <span
                  aria-hidden="true"
                  className={`rincian__dot-item block transition-all duration-300 ${
                    i === kini
                      ? "w-3.5 bg-white shadow-xs sm:w-5"
                      : "w-[5px] bg-white/50 hover:bg-white/80 sm:w-2"
                  }`}
                />
              </button>
            ))}
